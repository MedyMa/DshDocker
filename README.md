...
> The installer **shell** is a 32-bit PE — NSIS has no 64-bit implementation; that says nothing about
> the installed app. The bundled `fastlist-0.3.0-x86.exe` and `win10-arm64/OpenConsole.exe` are pnpm's
> and node-pty's own multi-arch payloads, so the gate only warns about them.
